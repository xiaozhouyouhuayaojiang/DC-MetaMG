...
if __name__ == "__main__":
    ...
    with open('data/MetaPath_drug.txt', 'w') as f:
        for row in DDI:
            f.write(' '.join(map(str, row)) + '\n')
    print(f"Drug meta-path saved: {DDI.sum()} edges")

    # ── miRNA meta-path ───────────────────────────────────────────────────────
    mirna_disease_matrix = np.loadtxt('data/array_MD.txt', dtype=int)
    num_mi = mirna_disease_matrix.shape[0]

    MMI = getMetaPath(
        num=num_mi,
        object_disease_matrix=mirna_disease_matrix,
        correlation_file='data/correlation_MiRNA.txt',
        max_hops=2,
        alpha=2.0,
        beta=1.0,
    )

    with open('data/MetaPath_miRNA.txt', 'w') as f:
        for row in MMI:
            f.write(' '.join(map(str, row)) + '\n')
    print(f"miRNA meta-path saved: {MMI.sum()} edges")
